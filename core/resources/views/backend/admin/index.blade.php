    ...
    {{-- <div class="row mb-4">
        ...
        {{--  <div class="col-md">
            <div class="d-flex border">
                ...
                <div class="flex-grow-1 bg-white p-4 w-100" style="height: 113px">
                    <a href="{{route('backend.admin.reservation','offline')}}"  class="text-uppercase text-secondary mb-0 "> <small> Reservations</small></a>
                    <h3 class="font-weight-bold mb-0">
        ...
        <div class="col-md">
            <div class="d-flex border">
                ...
                <div class="flex-grow-1 bg-white p-4 w-100" style="height: 113px">
                    ...
                    <h3 class="font-weight-bold mb-0">
                        <a href="{{auth()->guard('admin')->user()->can_access('admin')?route('backend.admin.room'):'#0'}}" class="text-muted">{{\App\Model\Room::distinct('number')->count('number')}}</a>
                    </h3>
                </div>
            </div>
        </div>
           ...
           <div class="card">
               ...
               <h4> Room Status
                   ...
                   <div class="float-md-right">
                       <a href="#" class="btn btn-sm btn-square btn-room btn-primary mr-1">Available</a>
                           ...
                           <form class="form-inline float-right">
                               <div class="form-group">
                                   <div class="input-group">
                                       <input name="date" type="text" id="date" value="{{ $data['date'] }}" class="form-control" readonly>
                                   </div>
                               </div>
                           ...
                           <table class="table table-bordered mb-0">
                               ...
                               @foreach($data['floor_plan'] as $floor)
                                   <tr>
                                       ...
                                       <td>
                                           @foreach(\App\Model\Room::where('floor_id', $floor->id)->distinct('number')->orderBy('number')->pluck('number') as $room)

                                               @if($ability = \App\Model\Room::where('number', $room)->first()->available_new($data['date']))
                                                   ...
                                                   <a href="#" class="btn btn-lg btn-square btn-room btn-danger mr-1 mt-1">{{$room}}</a>
                                               @else
                                                   <a href="#" class="btn btn-lg btn-square btn-room btn-primary mr-1 mt-1">{{$room}}</a>
                                               @endif
                                           @endforeach
                                       </td>
                                   </tr>
                       ...
                       <div role="tabpanel" class="tab-pane" id="room_type">
                           <table class="table table-bordered mb-0">
                               ...
                               @foreach($data['room_type'] as $room_type)
                                   <tr>
                                        @if($room_type->id%4==1)
                                            <td class="align-content-center font-weight-bold">{{ explode('-', $room_type->title)[0] }}</td>
                                            <td>
                                                    @forelse(\App\Model\Room::where('room_type_id', $room_type->id)->distinct('number')->orderBy('number')->pluck('number') as $room)
                                                        @if(\App\Model\Room::where('number', $room)->first()->available_new($data['date']))
                                                            <a href="#" class="btn btn-lg btn-square btn-room btn-danger mr-1 mt-1">{{$room}}</a>
                                                        @else
                                                            <a href="#" class="btn btn-lg btn-square btn-room btn-primary mr-1 mt-1">{{$room}}</a>
                                                        @endif
                        ...
                        <table class="table table-sm">
                            ...
                            @forelse($data['current_reservation'] as $key=>$current_reservation)
                                ...
                                <td><a href="{{route('backend.admin.reservation.view',$current_reservation->id)}}">{{$current_reservation->uid}}</a></td>
                                    ...
                                    <td>{{$current_reservation->guests()}}</td>
                        ...
                        <table class="table table-sm">
                            ...
                            @forelse($data['upcoming_reservation'] as $key=>$upcoming_reservation)
                                <tr>
                                    ...
                                    <td>{{$upcoming_reservation->guests()}}</td>
                                {{-- <td>{{$current_reservation->guest->email}}</td>
                                <td>{{$current_reservation->guest->phone}}</td> --}}
                                </tr>
                            @empty
