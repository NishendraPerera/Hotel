...
class ReservationController extends Controller
{
    ...
    public function reservation_list($booking_type = null, Request $request)
    {
        $reservations = $this->reservation;
        // if(null !== $booking_type){
        //     if(!in_array($booking_type,['online','offline']))
        ...
        $reservations=$reservations->orderBy('check_in', 'desc')
        ->where('check_out', '>', date('Y-m-d H:i:s', strtotime("-7 days", strtotime(date('Y-m-d')))))
        ...
        return Datatables::of($reservations)
                ...
                ->addColumn('payment_status_text', function($row){
                    $btn = $row->paymentStatus()['status'];
                    return $btn;
                })
    ...
    public function view($id){
        ...
        $paid_services = $this->paidService->whereStatus(1)->orderBy('title')->get();
        $foods = $this->food->whereStatus(1)->orderBy('title')->get();
        $staffs = $this->staff->whereStatus(1)->orderBy('first_name')->get();
        $gateways = $this->gateway->whereStatus(1)->where('is_offline',1)->get();
        return view('backend.admin.reservation.view',compact('reservation','gateways','paid_services', 'foods', 'staffs'));
    }
    ...
    public function changeStatus ($id,$status){
        $reservation = $this->reservation->findOrFail($id);
        if(!in_array($status,['pending','success','cancel','completed']))
            abort(401);
        ...
        $log_info = "Reservation Status Changed. From: $old_status, To: $reservation->status";
    ...
    public function payment(Request $request,$id){
       ...
       $reservation = $this->reservation->findOrFail($id);
        ...
        $log_info = "Payment Added. Type: $gateway_name, Amount: $request->amount";
        $this->logging($id, $log_info);
    ...
    public function addService(Request $request,$id){
       ...
       $paid_service = $this->paidService->findOrFail($request->service);
        ...
        $service->staff_id = $request->staff;
        ...
        $log_info = "Paid Service Added. Staff: $staff, Item: $paid_service->title, Rate: $paid_service->price, Qty: $request->qty, Amount: $service->price";
        $this->logging($id, $log_info);
    ...
    public function addFood(Request $request,$id){
        ...
        $reservation = $this->reservation->findOrFail($id);
        $food = $this->food->findOrFail($request->food);
        $service = new $this->res_food;
        $service->date = $request->date;
        $service->reservation_id =$reservation->id;
        $service->food_id = $request->food;
        $service->description = $request->description;
        $service->staff_id = $request->staff;
        $service->value = $food->price;
        $service->qty = $request->qty;
        $service->price = $food->price*$request->qty;
        $service->save();

        $staff = $service->staff->full_name;

        $log_info = "Food/ Beverage Added. Staff: $staff, Item: $food->title, Rate: $food->price, Qty: $request->qty, Amount: $service->price";
        $this->logging($id, $log_info);

        return back()->with('success','Food/ Beverage added Successful');
    }

    public function removeService($id){
        $reservation = $this->res_paid_service->findOrFail($id);

        $staff = $reservation->staff->full_name;
        $service =  $reservation->service;

        $log_info = "Service Removed. Staff: $staff, Item: $service->title, Rate: $reservation->value, Qty: $reservation->qty, Amount: $reservation->price";
        $this->logging($reservation->reservation_id, $log_info);
    ...
    public function removeFood($id){
        $reservation = $this->res_food->findOrFail($id);

        $staff = $reservation->staff->full_name;
        $food =  $reservation->food;

        $log_info = "Food Removed. Staff: $staff, Item: $food->title, Rate: $reservation->value, Qty: $reservation->qty, Amount: $reservation->price";
        $this->logging($reservation->reservation_id, $log_info);

        $reservation->delete();

        return back()->with('success','Food/ Beverage remove Successful');
    }
    ...
    public function removePayment($id){
        $reservation = $this->payment->findOrFail($id);

        $gateway_name = $reservation->gateway->name;

        $log_info = "Payment Removed. Type: $gateway_name, Amount: $reservation->amount";
        $this->logging($reservation->reservetion_id, $log_info);
